<%-- 
    Document   : link-edit
    Created on : Oct 6, 2008, 2:12:20 PM
    Author     : James A Hill
--%><%@page contentType="text/html" pageEncoding="UTF-8" import="java.util.*, org.tranche.util.*, org.proteomecommons.www.link.*, org.proteomecommons.www.reference.*"%><%
...
if (link == null) {
    ...
    request.setAttribute("pageTitle", "Link - " + link.title + " - Edit");
}
...
if (link != null) {
    if (link.linkID != link.linkID) {
        %>You do not have the right to add references to this link.<%
        link = null;
    }
}
...
if (link != null) {
        ...
        try {
            ...
            for (Enumeration e = request.getParameterNames(); e.hasMoreElements();) {
                ...
                if (parameterName.startsWith("refName")) {
                    ...
                    ReferenceUtil.addReference("link", "linkID", link.linkID, fieldTable, fieldName, Integer.valueOf(fieldID));
                }
            }
            %>
            <script language="javascript">
                document.location.href = 'link.jsp?i='+<%=link.linkID%>;
            </script>
            <%
        } catch (Exception e) {
            error = e.getMessage();
    ...
    %>
    <form action="link-edit.jsp" method="post">
        <input type="hidden" name="i" value="<%=link.linkID%>"/>
        <input type="hidden" name="action" value="edit"/>
        <div class="table">
            <div class="table-row">
                <div class="table-cell">Title</div>
                <div class="table-cell"><%=link.title%></div>
                <div class="table-cell"><div>Maximum length: 100 characters.</div></div>
            </div><div class="table-row">
                <div class="table-cell">Description</div>
                <div class="table-cell"><%=link.description%></div>
            </div><div class="table-row">
                <div class="table-cell">URL</div>
                <div class="table-cell"><%=link.url%></div>
                <div class="table-cell"><div>Maximum length: 255 characters.</div></div>
            </div><div class="table-row">
                <div class="table-cell">References</div>
                <div class="table-cell">
                    <%
                    List <Reference> referencesFrom = ReferenceUtil.getReferencesFrom("link", "linkID", link.linkID);
                ...
                <div class="table-cell"><input type="submit" value="Edit Link"></div>
            </div>
        </div>
    </form>
    <%
