<%-- 
    Document   : my-news
    Created on : Oct 21, 2008, 3:47:47 PM
    Author     : James A Hill
--%><%@page contentType="text/html" pageEncoding="UTF-8" import="java.util.*, org.proteomecommons.www.news.*, org.tranche.util.*"%><%
request.setAttribute("pageTitle", "My News");
...
if (user != null) {
    ...
    int count = NewsUtil.getNewsCount(filter, true, user.userID);
    int pageNum = 1;
    ...
    <div class="table">
        <div class="table-row">
            <div class="table-cell">
                <div class="table" style="vertical-align: top; border-bottom: 1px solid #333;">
                    <div class="table-row">
                        <div class="table-cell" style="font-size: 15pt; font-weight: bold;">My News</div><div class="table-cell" style="text-align: right">
                            <form action="my-news.jsp" method="get" id="browseNewsForm">
                                <div>
                                    ...
                                    <b><%=count%></b> news items found that match "<%=MySQLDatabaseUtil.formatTextForDatabase(filter)%>".
                                </div>
                            </form>
                        </div>
                    </div>
        ...
        <%
        List<News> news = NewsUtil.getNews(filter, orderBy, "ASC", pageNum, perPage, true, user.userID);
